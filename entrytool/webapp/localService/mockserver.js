    ...
    return {
        ...
        init: function () {
            var oUriParameters = jQuery.sap.getUriParameters(),
                ...
                sEntity = 'ItemCategory',
                sEntity2 = "SessionUser",
                sEntity3 = "CustomerDetails",
                sEntity4 = "OrderStatusCount",
                sEntity5 = "OrderHeaderDetails",
                sEntity6 = "OTCHealthCareReport",
            ...
            if (sErrorParam) {
                aRequests.forEach(function (aEntry) {
                    ...
                    else if (aEntry.path.toString().indexOf(sEntity2) > -1) {
                        fnResponse(iErrorCode, sErrorParam, aEntry);
                    }
                    else if (aEntry.path.toString().indexOf(sEntity3) > -1) {
                        fnResponse(iErrorCode, sErrorParam, aEntry);
                    }
                    else if (aEntry.path.toString().indexOf(sEntity4) > -1) {
                        fnResponse(iErrorCode, sErrorParam, aEntry);
                    }
                    else if (aEntry.path.toString().indexOf(sEntity5) > -1) {
                        fnResponse(iErrorCode, sErrorParam, aEntry);
                    }
                    else if (aEntry.path.toString().indexOf(sEntity6) > -1) {
                        fnResponse(iErrorCode, sErrorParam, aEntry);
                    }
                });
            }
